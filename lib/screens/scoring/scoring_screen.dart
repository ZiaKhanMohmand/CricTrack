...
class _ScoringScreenState extends State<ScoringScreen> {
  ...
  Future<void> _askNextBatsman() async {
    ...
    final remaining = inn.battingTeam.players
        .where((p) => !p.isOut && p != inn.nonStriker)
        .toList();

    if (remaining.isEmpty) {
      // last man stands: nonStriker bats alone
      if (match.lastManStands && inn.nonStriker != null) {
        provider.setLastManBatsAlone();
        return; // don't end innings — last man still bats
      }
      _checkInningsEnd();
      return;
    }

    final next = await _showPlayerPicker('Next Batsman', remaining);
    if (next == null) return;
  ...
  void _onBallTapped(BallEvent event) async {
    ...
    // ADD: check target chased immediately
  ...
  void _checkInningsEnd() {
    if (_inningsEndHandled) return; // ADD THIS
    final provider = context.read<MatchProvider>();
    if (provider.isInningsOver) {
      _inningsEndHandled = true; // ADD THIS
        ...
        actions: [
          ElevatedButton(
            ...
            onPressed: () {
              ...
              _inningsEndHandled = false; // reset guard
              _setupOpeners();
            },
            child: const Text(
              'Start 2nd Innings',
                    ...
                    Row(
                      children: [
                        ...
                        _extraBtn(
                          ...
                          Colors.black,
                          () =>
                              _onBallTapped(BallEvent(isNoBall: true, runs: 1)),
                        ),
                      ],
                    ),
                    const SizedBox(height: 8),
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: GestureDetector(
                        onTap: () => context.read<MatchProvider>().swapStrike(),
                        child: Container(
                          height: 50,
                          decoration: BoxDecoration(
